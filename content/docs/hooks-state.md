...
הדוגמה החדשה שלנו מתחילה בייבוא של ה- Hook `useState` מריאקט:
...
**מה זה Hook?** Hook זה פונקציה מיוחדת שנותנת לך "להתחבר"  לפיצ'רים של ריאקט. לדוגמה, `useState` הוא Hook שנותן לך להוסיף state לקומפוננטות פונקציונליות. אנו נלמד על Hooks אחרים בהמשך.
...
**מה קריאה ל- useState עושה?** זה מגדיר "משתנה state". המשתנה שלנו נקרא `count` אך נוכל לקרוא לו בכל שם אחר, כמו `banana`. זוהי דרך "לשמור" על ערכים בין קריאות פונקציה -- `useState` היא דרך חדשה להשיג את אותן מטרות שהשגנו באמצעות `this.state` במחלקה. בדרך כלל, משתנים "נעלמים" כשהפונקציה מסיימת את פעולתה אבל משתני state נשמרים על ידי ריאקט.
...
אנו מגדירים משתנה state חדש בשם `count`, וקובעים אותו כ- `0`. ריאקט יזכור את הערך נוכחי בין רינדורים, ויספק את הערך האחרון לפונקציה שלנו. אם אנחנו רוצים לעדכן את ה- `count` הנוכחי, נקרא ל- `setCount`.
...
* **שורה 1:**  אנו מייבאים את `useState` hook מריאקט. זה נותן לנו לשמור state מקומי בתוך קומפוננטה פונקציונלית. 
...
* **שורה 9:** כשהמשתמש לוחץ על הכפתור, נקרא ל- `setCount` עם ערך חדש. לאחר מכן ריאקט ירנדר מחדש את הקומפוננטה `Example`, ויעביר את ערך ה- `count` החדש אליה.
...
השמות משמאל הם לא חלק מה- API של ריאקט. אתה יכול לקרוא להם בכל שם שתרצה:
...
>הערה
>
>יכול להיות שאתה סקרן לדעת איך ריאקט יודע איזה קומפוננטה מתאימה ל- `useState` בגלל שאנחנו לא מעבירים דברים כמו `this` חזרה לריאקט. נענה על [שאלה זו](/docs/hooks-faq.html#how-does-react-associate-hook-calls-with-components) ואחרות רבות בעמוד שעונה על שאלות נפוצות.
...
בעמוד זה למדנו על אחד מה- Hooks שמסופקים על ידי ריאקט, שנקרא `useState`. אנו הולכים להתייחס אליו גם כה- "State Hook". זה נותן לנו להוסיף state מקומי לקומפוננטות פונקציונליות בריאקט – מה שעשינו בפעם הראשונה אי פעם!

למדנו גם קצת על מה הם Hooks. Hooks הם פונקציות שנותנות לך "להתחבר" לפיצ'רים של ריאקט מתוך קומפוננטות פונקציונליות. השמות שלהם תמיד מתחילים עם `use`, ויש עוד Hooks שלא ראינו עדיין.

...
`setState()` תמיד יוביל לרינדור מחדש אלא אם `shouldComponentUpdate()` תחזיר `false`. אם אוביקטי מיוטבל נמצאים בשימוש ולוגיקת רינדור מותנה לא יכולה להיות ממומשת ב- `shouldComponentUpdate()`, קריאה ל- `setState()` רק כאשר ה-state החדש שונה מהקודם תמנע רינדור מחדש מיותר.

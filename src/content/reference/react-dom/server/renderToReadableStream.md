...
`renderToReadableStream` מעבד עץ React ל[זרם אינטרנט קריא.](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream)
...
API זה תלוי ב-[זרמי אינטרנט.](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) עבור Node.js, use [`renderToPipeableStream`](/reference/react-dom/server/renderToPipeableStream) במקום זאת.
...
התקשר ל-`renderToReadableStream` כדי לעבד את עץ ה-React שלך כ-HTML לתוך [זרם אינטרנט קריא.](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream)
...
* **אופציונלי** `options`: אובייקט עם אפשרויות סטרימינג.
  ...
  * **אופציונלי** `onError`: התקשרות חוזרת הנפתחת בכל פעם שיש שגיאת שרת, בין אם [ניתנת לשחזור](#מתאושש-from-errors-outside-the-shell) או [לא.](#recovering-from-errors-inside-the-shell) כברירת מחדל, זה קורא רק __.K_T אם תעקוף אותו ל[יומן דוחות קריסה,](#logging-crashes-on-the-server) ודא שאתה עדיין קורא ל-`console.error`. אתה יכול גם use אותו כדי [להתאים את קוד המצב](#setting-the-status-code) לפני שהקליפה תיפלט.
  ...
  * **אופציונלי** `signal`: [אות ביטול](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal) המאפשר לך [להפסיק את עיבוד השרת](#aborting-server-rendering) ולעבד את השאר בלקוח.
...
- אם רינדור ה-[shell](#specificing-what-goes-into-the-shell) יצליח, ההבטחה הזו תיפתר ל-[זרם אינטרנט קריא.](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream)
- אם עיבוד הקליפה נכשל, ההבטחה תידחה. [השתמש בזה כדי להפיק מעטפת חלופית.](#מתאוששים-from-errors-inside-the-shell)

לזרם המוחזר יש מאפיין נוסף:

* `allReady`: הבטחה שנפתרת כאשר כל העיבוד הושלם, כולל הן את [המעטפת](#specificing-what-goes-into-the-shell) וכל [התוכן הנוסף.](#streaming-more-content-as-it-loads) אתה יכול `await stream.allReady` לפני החזרת תגובה לסורקים [עבור סורקים]. generation.](#waiting-for-all-content-to-load-for-crawlers-and-static-generation) אם תעשה זאת, לא תקבל שום טעינה מתקדמת. הזרם יכיל את ה-HTML הסופי.

---

## שימוש {/*usage*/}

### עיבוד עץ React כ-HTML לזרם אינטרנט קריא {/*rendering-a-react-tree-as-html-to-a-readable-web-stream*/}
...
על ידי [חלוקת האפליקציה שלך](#specificing-what-goes-into-the-shell) לתוך המעטפת (מעל כל גבולות `<Suspense>`) ושאר התוכן, כבר פתרת חלק מהבעיה הזו. אם המעטפת שגיאה, בלוק `catch` שלך יפעל, מה שמאפשר לך להגדיר את קוד מצב השגיאה. אחרת, אתה יודע שהאפליקציה עשויה להתאושש בלקוח, כך שתוכל לשלוח "אישור".
...
אם רכיב *מחוץ* למעטפת (כלומר בתוך גבול `<Suspense>`) זורק שגיאה, React לא יפסיק לעבד. המשמעות היא שההתקשרות חוזרת `onError` תידלק, אבל הקוד שלך ימשיך לפעול מבלי להיכנס לבלוק `catch`. הסיבה לכך היאuse React ינסה להתאושש מהשגיאה הזו בלקוח, [כמתואר לעיל.](#recovering-from-errors-outside-the-shell)
...
אתה יכול [ליצור תת-מחלקות `Error` משלך](https://javascript.info/custom-errors) ו-use האופרטור [`instanceof`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/instanceof) כדי לבדוק איזו שגיאה נזרק. לדוגמה, אתה יכול להגדיר `NotFoundError` מותאם אישית ולזרוק אותו מהרכיב שלך. לאחר מכן תוכל לשמור את השגיאה ב-`instanceof` ולעשות משהו שונה לפני החזרת השגיאה: `onError`
